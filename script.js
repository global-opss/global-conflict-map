...
fetch('https://raw.githubusercontent.com/datasets/geo-boundaries-world-110m/master/countries.geojson')
    ...
    const strategicAssets = [
        ...
        { name: "USS Gerald R. Ford", type: "us-naval", lat: 15.5000, lon: 41.8000, description: "Carrier Strike Group 12. Operating near Israel, Mission still unknown" },
...
(function() {
    const startGlobalWar = () => {
       ...
       const MISSION_DATA = [
    // 1. ИЗРАЕЛ -> ТЕХЕРАН (Удар по центъра на IRGC)
    { f: [31.50, 34.75], target: [35.69, 51.30], c: "#00ebff", s: "heavy" },
    
    // 2. ИЗРАЕЛ -> НАТАНЗ (Пробив в ядрения комплекс)
    { f: [31.00, 35.00], target: [33.72, 51.72], c: "#00ebff", s: "heavy" },

    // 3. САЩ -> БАНДАР АБАС (Атака по иранския флот в залива)
    { f: [25.27, 51.50], target: [27.15, 56.25], c: "#00ebff", s: "fast" },

    // 4. ИРАН -> КУВЕЙТ (Посолство на САЩ)
    { f: [30.50, 48.00], target: [29.35, 47.95], c: "#ff1100", s: "gulf" },

    // 5. ИРАН -> ДУБАЙ (Пристанище Jebel Ali)
    { f: [28.40, 55.90], target: [25.25, 55.30], c: "#ff1100", s: "gulf" },

    // 6. ИРАН -> ЕРБИЛ (Удар по кюрдската столица)
    { f: [35.50, 47.00], target: [36.19, 44.01], c: "#ff1100", s: "fast" },

    // 7. ИЗРАЕЛ -> ДАМАСК (Ликвидиране на пратка оръжие)
    { f: [32.00, 34.80], target: [33.51, 36.27], c: "#00ebff", s: "heavy" },

    // 8. ИРАН -> ТЕЛ АВИВ (Единична ракета, преодоляла ПВО)
    { f: [34.40, 46.50], target: [32.0853, 34.7818], c: "#ff1100", s: "heavy" },

    // 9. ИРАН -> РИЯД (Предупредителен удар по Саудитска Арабия)
    { f: [38.07, 46.29], target: [24.6813, 46.6214], c: "#ffcc00", s: "heavy" },

    // 10. БРИТАНИЯ (КИПЪР) -> ИРАН (Разузнавателно-ударен дрон)
    { f: [34.59, 32.98], target: [29.00, 50.00], c: "#00ebff", s: "fast" },

    { f: [35.10, 47.50], target: [36.19, 44.01], c: "#ff1100", s: "fast" },

    { f: [36.00, 44.00], target: [35.50, 46.50], c: "#00ebff", s: "heavy" },       
];
