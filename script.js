...
(function() {
    ...
    // --- ГАРАНТИРАН КОНТРОЛЕР ЗА ЛОГА --- [cite: 2026-02-20]
(function forceLogControl() {
    const findAndAttach = () => {
        // Търсим всички DIV-ове, които съдържат "ESCALATION LOG" [cite: 2026-02-20]
        const target = Array.from(document.querySelectorAll('div')).find(el => 
            el.textContent.includes('ESCALATION LOG') && el.style.position === 'absolute'
        );

        if (target && !document.getElementById('logToggleBtn')) {
            target.style.transition = "all 0.4s cubic-bezier(0.4, 0, 0.2, 1)";
            target.style.overflow = "hidden";
            
            const btn = document.createElement('div');
            btn.id = "logToggleBtn";
            // Дизайн в стила на терминала [cite: 2026-02-20]
            btn.innerHTML = "[ – ]"; 
            btn.style = `
                position: absolute; top: 5px; right: 10px; 
                color: #ff4500; cursor: pointer; font-family: monospace;
                font-weight: bold; z-index: 1000; background: rgba(0,0,0,0.5);
                padding: 2px 5px; border: 1px solid #ff4500;
            `;

            let collapsed = false;
            const originalHeight = target.offsetHeight + "px";
            ...
            console.log("✅ Лог контролерът е активиран, шефе!"); [cite: 2026-02-20]
        } else {
            // Ако логът още не е заредил, опитай пак след 2 секунди [cite: 2026-02-20]
