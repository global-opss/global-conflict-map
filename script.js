...
const blueZone = ['France', 'Germany', 'United Kingdom', 'Italy', 'Poland', 'Bulgaria', 'Romania', 'Greece', 'Norway', 'Jordan', 'Turkey', 'Saudi Arabia', 'Lithuania', 'Belarus', 'Finland', 'Sweden'];
...
const tripwireCoords = [
    [28.15, 50.10], // Начало дълбоко в Персийския залив (близо до Кувейт/Саудитска Арабия)
    [27.50, 52.30], 
    [27.00, 54.50], 
    [26.65, 56.10], // Критичната точка в Ормуз
    [26.35, 56.55], 
    [25.90, 57.20], 
    [25.10, 58.40], 
    [24.00, 59.90], 
...
const adenPathCoords = [
    // --- СЕВЕРЕН СЕКТОР (Средиземно море надолу) ---
    [31.25, 32.33], // Порт Саид (Изход)
    [29.93, 32.55], // Суец (Вход канал)
    [25.30, 35.80], // Червено море (Север)
    [20.10, 38.90], // Червено море (Център)
    [15.50, 41.50], // Червено море (Юг)
    
    // --- ЮЖЕН СЕКТОР (Протока и Аденския залив) ---
...
// Обновяваме дебелината и сегментите, за да изглежда по-мащабно
const adenLine = L.polyline(adenPathCoords, {
    color: '#00F2FF',
    weight: 9,              // Малко по-дебела за по-добър завой
    opacity: 1.0,          
    dashArray: '40, 35',    // По-дълги сегменти, за да не се начупва на завоя
